...
from PyQt6.QtCore import QVariantAnimation, QSettings
...
#view page
    ...
    def generate_list(self):
        ...
        query = "SELECT song_id, artist, song, year FROM songs WHERE 1=1"
        parameters = []
        ...
        for row_data in results:
            ...
            song_id, artist, title, year = row_data
            genres = self.db.get_song_genres(song_id)

            self.ui.song_table.setItem(row, 0, QTableWidgetItem(artist))
            self.ui.song_table.setItem(row, 1, QTableWidgetItem(title))
            self.ui.song_table.setItem(row, 2, QTableWidgetItem(genres))
            self.ui.song_table.setItem(row, 3, QTableWidgetItem(year))
    ...
    def apply_view_filters(self):
        ...
        query = """
            SELECT s.artist, s.album, s.song, GROUP_CONCAT(g.genre), s.year
            FROM songs s
            JOIN song_genres g ON s.song_id = g.song_id
            WHERE 1=1
        """
        params = []
    ...
    def show_stats_popup(self):
        ...
        if user_status == "Admin":
            query = "SELECT genre FROM song_genres"
    ...
    def user_table(self):
        ...
        for row_data in songs:
            ...
            song_id_query = self.db.conn.execute(
                "SELECT song_id FROM songs WHERE artist = ? AND album = ? AND song = ?",
                (row_data[0], row_data[1], row_data[2])
            ).fetchone()

            genres = self.db.get_song_genres(song_id_query[0]) if song_id_query else ""

            for col, data in enumerate(row_data):
                self.ui.profile_table.setItem(row, col, QTableWidgetItem(str(data)))
            self.ui.profile_table.setItem(row, 3, QTableWidgetItem(genres))  # overwrite genre column
    ...
    def save_song(self):
        ...
        genres = [item.text() for item in self.ui.genre_list_add.selectedItems()]

        if not artist or not album or not song:
            QMessageBox.warning(self, "Input Error", "Please fill in Artist, Album, and Song fields.")
            return
        if not genres:
            QMessageBox.warning(self, "Genre Missing", "Please select at least one genre.")
            return
        try:
            year_int = int(year)
            if year_int < 1700 or year_int > 2026:
                raise ValueError
        except ValueError:
            QMessageBox.warning(self, "Invalid Year", "Please enter a valid year between 1700 and 2026.")
            return

        # Insert song first
        self.db.add_song(self.current_user["id"], artist, album, song, "", year, lyrics)

        # Get inserted song ID
        song_id = self.db.conn.execute("SELECT last_insert_rowid()").fetchone()[0]

        # Insert each genre
        for genre in genres:
            self.db.conn.execute("INSERT INTO song_genres (song_id, genre) VALUES (?, ?)", (song_id, genre))
    ...
    def load_table(self):
        query = "SELECT song_id, artist, album, song, year FROM songs"
        songs = self.db.conn.execute(query).fetchall()
        self.ui.list_table.setRowCount(0)
        ...
        for row_data in songs:
            ...
            song_id, artist, album, song, year = row_data
            genres = self.db.get_song_genres(song_id)

            self.ui.list_table.setItem(row, 0, QTableWidgetItem(artist))
            self.ui.list_table.setItem(row, 1, QTableWidgetItem(album))
            self.ui.list_table.setItem(row, 2, QTableWidgetItem(song))
            self.ui.list_table.setItem(row, 3, QTableWidgetItem(genres))
            self.ui.list_table.setItem(row, 4, QTableWidgetItem(year))
